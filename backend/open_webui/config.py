...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.BLDRCore.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.BLDRCore.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.BLDRCore.com{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (BLDR Core + Ollama)
